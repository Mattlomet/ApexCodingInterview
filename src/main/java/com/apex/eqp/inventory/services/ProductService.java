...
public class ProductService {
    ...
    public Collection<Product> getAllProduct() {
        Set<String> namesOfRecalledProducts = recalledProductService.getAllRecalledProducts().stream().map(
            RecalledProduct::getName).collect(Collectors.toSet());

        ProductFilter filter = new ProductFilter(namesOfRecalledProducts);
